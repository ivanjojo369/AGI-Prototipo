...
class UnifiedMemory:
    """
    Memoria vectorial con FAISS (CPU/GPU opcional).

    Fase 1:
      - Etiquetas por recuerdo: result_quality, confidence, trace_id
      - get_status(): contadores y latencias avg/p95
    Cierre fino:
      - mem_id estable por recuerdo (persistente) y devuelto como "citation_id"
    """
    ...
    def _init_faiss_index(self):
        try:
            if os.path.exists(self.index_file):
                ...
                self.logger.info("[UM] Índice FAISS cargado.")
            else:
                self.index = faiss.IndexFlatL2(self.vector_dim)
                self.logger.info("[UM] Nuevo índice FAISS.")
            if self.use_gpu:
                try:
            ...
            self.logger.error(f"[UM] Error FAISS: {e}")
            self.index = faiss.IndexFlatL2(self.vector_dim)
    ...
    def _load_memories(self):
        try:
            if os.path.exists(self.vector_file):
                ...
                # migración: generar mem_id/labels si falta
                changed = False
                for m in self.memories:
                    if "mem_id" not in m:
                        m["mem_id"] = f"mem://{uuid4()}"
                        changed = True
